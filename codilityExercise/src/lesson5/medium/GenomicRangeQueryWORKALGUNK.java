...
// Each nucleotide has an impact factor, which is an integer. Nucleotides of types A, C, G and T have impact factors of
...
//To solve this problem efficiently, you can preprocess the DNA sequence to calculate the cumulative occurrences of each nucleotide at each position. Then, for each query (P[K], Q[K]),
//        you can use the preprocessed information to determine the minimal impact factor within the specified range.
public class GenomicRangeQueryWORKALGUNK {
    public int[] solution(String S, int[] P, int[] Q) {
        int N = S.length();
        int M = P.length;

        // Create arrays to store cumulative occurrences of each nucleotide
        ...
        for (int k = 0; k < M; k++) {
            ...
            if (prefixA[end] - prefixA[start] > 0) {
                result[k] = 1; // 'A' has the lowest impact factor
            } else if (prefixC[end] - prefixC[start] > 0) {
                result[k] = 2;
            } else if (prefixG[end] - prefixG[start] > 0) {
                result[k] = 3;
            } else {
                result[k] = 4; // 'T' has the highest impact factor
            }
        }
    ...
    public static void main(String[] args) {
        GenomicRangeQueryWORKALGUNK solution = new GenomicRangeQueryWORKALGUNK();
        int [] P = {2, 5, 0};
        int [] Q = {4, 5, 6};
        String S = "CAGCCTA";

        int [] answers = solution.solution(S, P, Q);
...
//    This solution has a time complexity of O(N + M), where N is the length of the DNA sequence and M is the number of queries.
//        The space complexity is O(N) to store the cumulative occurrences of each nucleotide.