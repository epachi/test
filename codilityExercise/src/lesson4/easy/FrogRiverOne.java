...
//In this solution, we iterate through the array A representing the falling leaves.
//        We use a HashSet positions to keep track of unique positions where leaves have fallen.
//        As soon as the size of the HashSet becomes equal to X (the target position on the other side of the river),
//        we return the current time. If the loop completes without finding a solution, we return -1.
//
//        This solution has a time complexity of O(N) where N is the length of the array A,
//        and the space complexity is O(X) due to the HashSet.
//        The space complexity could be O(1) in the best case if the target position is reached early in the iteration.