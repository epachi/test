...
//Key Insights
//
//Search Space: The minimum possible large sum is the maximum element in the array (when each large element gets its own block). The maximum possible large sum is the total sum of all elements (when we use only 1 block).
//Monotonic Property: If we can divide the array into K blocks with a maximum sum of X, we can also do it with any value greater than X. This monotonic property allows us to use binary search.
//
//        Algorithm Steps
//
//Binary Search Setup:
